class PokemonTCGCatalog {
    ...
    createCard({ name, number, imageUrl, supertype, subtype, rarity, id }){
        ...
        <article class="card" id=${id} data-card>
            ...
            <p class="card__description"><span class="bold">Supertype: ${supertype}</span></p>
            <p class="card__description"><span class="bold">Subtype: ${subtype}</span></p>
            <p class="card__description ${rarity ? '' : 'hide'}"><span class="bold">Rarity: ${rarity}</span></p>
        </article>`;
